...
from DDS_UNet.LDR import LDR
from DDS_UNet.MSM import MSM
from DDS_UNet.GAT import GAT1, GAT2, GAT3, GAT4, GATBottleneck, DeepGATBottleneck
...
class DDS_UNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, deep_supervision=False, img_size=384, **kwargs):
        super().__init__()
        ...
        self.Convstage1 = LDR(input_channels, self.filters[0])
        ...
        # self.Sk1 = GAT4(self.filters[0], self.filters[0])
        # self.Sk2 = GAT3(self.filters[1], self.filters[1])
        # self.Sk3 = GAT2(self.filters[2], self.filters[2])
        # self.Sk4 = GAT1(self.filters[3], self.filters[3])

        self.Sk1 = GAT1(self.filters[0], self.filters[0])    # Shallow -> Simple (1-level)
        self.Sk2 = GAT2(self.filters[1], self.filters[1])    # Mid-shallow -> 2-level
        self.Sk3 = GAT3(self.filters[2], self.filters[2])    # Mid-deep -> 3-level
        self.Sk4 = GAT4(self.filters[3], self.filters[3])    # Deep -> Complex (4-level)
        ...
        # GAT Bottleneck for global context at deepest layer
        # OPTION 1: Shallow bottleneck (fast, single GAT layer)
        # self.gat_bottleneck = GATBottleneck(self.filters[4], self.filters[4], heads=8, reduction=2)
        
        # OPTION 2: Deep bottleneck (slower, 3 GAT layers with multi-scale)
        # Uncomment below and comment above to use deep version:
        self.gat_bottleneck = DeepGATBottleneck(self.filters[4], self.filters[4], heads=6)
    ...
    def forward(self, x):
        ...
        out = self.gat_bottleneck(out)  # Apply GAT at bottleneck

        ### Decoder
        out = self.Upstage1(out)
        out = self.Att_stageU4(out)
        sk4 = self.Sk4(t4)
        if out.shape[-2:] != sk4.shape[-2:]:
            sk4 = F.interpolate(sk4, size=out.shape[-2:], mode='bilinear', align_corners=False)
        out = torch.add(out, sk4)

        out = self.Upstage2(out)
        out = self.Att_stageU3(out)
        sk3 = self.Sk3(t3)
        if out.shape[-2:] != sk3.shape[-2:]:
            sk3 = F.interpolate(sk3, size=out.shape[-2:], mode='bilinear', align_corners=False)
        out = torch.add(out, sk3)

        out = self.Upstage3(out)
        out = self.Att_stageU2(out)
        sk2 = self.Sk2(t2)
        if out.shape[-2:] != sk2.shape[-2:]:
            sk2 = F.interpolate(sk2, size=out.shape[-2:], mode='bilinear', align_corners=False)
        out = torch.add(out, sk2)

        out = self.Upstage4(out)
        out = self.Att_stageU1(out)
        sk1 = self.Sk1(t1)
        if out.shape[-2:] != sk1.shape[-2:]:
            sk1 = F.interpolate(sk1, size=out.shape[-2:], mode='bilinear', align_corners=False)
        out = torch.add(out, sk1)
