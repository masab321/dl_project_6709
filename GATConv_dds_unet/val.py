...
resume = 'False'   # whether to resume
val_num = 1   # random seed for train/val split

#####################################################################################
models_name = "DDS_UNet"
...
# Only DDS_UNet is supported in this stripped repository
...
def main():
    ...
    if config['arch'] == 'DDS_UNet':
        model = DDS_UNet(config['num_classes'], config['input_channels'], config['deep_supervision'])
    else:
        raise NotImplementedError(f"Only DDS_UNet is supported. Got {config['arch']}")
